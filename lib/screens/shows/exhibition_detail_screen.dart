...
class _ExhibitionDetailScreenState extends State<ExhibitionDetailScreen> {
  ...
  Future<void> _load() async {
    try {
      ...
      setState(() {
        _ex = data['exhibition'] ?? data['data']?['exhibition'] ?? data['data'] ?? data;
        _loading = false;
      });
    } catch (e) {
  ...
  void _handleShare() {
    if (_ex != null) {
      Share.share('Check out "${_ex!['title']}"');
    }
  }
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Text('Delete Exhibition',
            style: TextStyle(color: AppColors.text, fontWeight: FontWeight.w300)),
                    ...
                    child: Wrap(
                      ...
                      children: [
                        ...
                        if (ex['location'] != null &&
                          ...
                          _infoPill(
                              Icons.location_on_outlined, ex['location']),
                        ...
                        if (ex['isFree'] == true)
                          ...
                          _infoPill(Icons.confirmation_num_outlined,
                              _formatPrice(ex['ticketPrice'], ex['currency'] ?? 'NOK')),
                        if (ex['views'] != null)
                          _infoPill(Icons.visibility_outlined,
                              '${ex['views']} views'),
                      ],
                    ),
                  ),
              ...
              child: img != null
                  ? Image.network(img,
                      ...
                      errorBuilder: (_, __, ___) => Container(
                          width: 140, height: 160, color: AppColors.surfaceDim))
                  : Container(
                      width: 140,
                      height: 160,
                      color: AppColors.surfaceDim,
